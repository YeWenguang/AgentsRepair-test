...
model3 = AutoModelForCausalLM.from_pretrained(model1_id)

# 在模型上启用自动混合精度
model1 = model3.half().to("cuda")

tokenizer2 = AutoTokenizer.from_pretrained(model2_id)
model4 = AutoModelForCausalLM.from_pretrained(model2_id)

# 在模型上启用自动混合精度
model2 = model4.half().to("cuda")
...
def compile_and_run_cpp(cpp_code):
    global total_tests
    compile_process = subprocess.run(['g++', '-x', 'c++', '-o', 'temp_executable', '-'], input=cpp_code, text=True,
                                     capture_output=True)
    if compile_process.returncode != 0:
        error_message = compile_process.stderr
        print("Compilation failed.")
        return False, error_message  # 返回编译失败标志和错误信息
    # probid = probid_content
    # Run the compiled executable with test cases
    # run_test_cases(cpp_code, probid)
    return True, None  # 返回编译成功标志和空的错误信息
    ...
    try:
        ...
        if compile_process.returncode == 0:
            ...
            process = subprocess.Popen('./temp', stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                                       text=True)
            stdout, stderr = process.communicate(input='\n'.join(input_data), timeout=15)  # 将输入数据转换为字符串形式并传递给子进程
...
def extract_error_info(error_messages, source_code):
    ...
    for match in matches:
        ...
        error_desc = match[2]
        # print(f"match[0]:{match[0]}, match[1]:{match[1]}, match[2]:{match[2]},match[3]:{match[3]}")
        # 更新错误行的内容
...
for idx, pseudocode in enumerate(pseudocodes[start_index:-1]):
    ...
    while epochs < 2 and not test_pass:
        # 代码生成
...
"""
                ...
                if not test_pass:
                    ...
                    while not success:
                        ...
                        user_query = f"""
### Instruction:
The following is pseudocode and incorrect C++program translation. Explain the original pseudocode, then translate the C++program line by line and make corrections using the provided feedback.
...
"""
    ...
    while not test_pass and 2 <= epochs < 4:
...
"""
        ...
        while not compile_pass or not test_pass:
            ...
            Cpp_explanations = """ This is a C++ program that solves a problem involving counting the number of ways to place 7 queens on a chessboard such that no queen attacks any other queen.
    Here's a line-by-line explanation of the program:
    1. `#include <iostream>`: This line includes the standard input/output library, which is used for input/output operations.
    2. `#include <vector>`: This line includes the vector library, which is used to store and manipulate arrays.
    [...]
            """
            if not compile_pass:
...
"""
                    ...
                    success, cpp_code1, probid_content = extract_cpp_code(decoded[0], start_index + idx + 1)
...
"""
                        ...
                        success, cpp_code2, probid_content = extract_cpp_code(decoded[0], start_index + idx + 1)
                        if success:
                            cpp_code = cpp_code2
